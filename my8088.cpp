...
int Key_Buffer[0x100];              //Buffer to hold key strokes
char Key_Head, Key_Tail;            //Pionters for keyboard buffer
...
int main()                                   
{
   ...
   char FFFF0[] = {0XEA, 0X00, 0X01, 0X00, 0XF0, 'E', 'M', ' ', '0', '4', '/', '1', '0', '/', '2', '0'};
   ...
   while (Stop_Flag != true)                          //Start access to memory/IO
   {
      ...
      {  
         ...
         { 
            usleep(500);                                   //Give the 8088 time to run
            if(Key_Head != Key_Tail)                        //Check for key stroke
            { 
...
void Insert_Key() //Interrupt_9
{
   ...
   Write_Memory_Byte(0x400 + Key_Buffer_Tail, character_codes[Key_Buffer[Key_Head]]); //Write Character code at the keyboard buffer tail pointer
   Write_Memory_Byte(0x401 + Key_Buffer_Tail, scan_codes[Key_Buffer[Key_Head]]);      //Write scan code at the keyboard buffer tail pointer 
